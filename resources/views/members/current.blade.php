...
@section('title', '日向坂46 現役メンバー一覧 | HINABASE')
@section('meta_description', '日向坂46の現役メンバー一覧ページです。現在在籍しているメンバーを期別に掲載し、各プロフィール詳細ページへ移動できます。')

@push('head_meta')
    <link rel="canonical" href="{{ route('members.current') }}">
    ...
    <script type="application/ld+json">
    {
        ...
        "@type":"CollectionPage",
        "name":"日向坂46 現役メンバー一覧",
        "url":"{{ route('members.current') }}",
        "isPartOf":{
            "@type":"WebSite",
            "name":"HINABASE",
            "url":"{{ url('/') }}"
        },
        "description":"日向坂46の現役メンバーを期別に掲載した一覧ページです。"
    }
    </script>
    ...
    <script type="application/ld+json">
    {
        ...
        "itemListElement":[
            ...
            {"@type":"ListItem","position":3,"name":"現役メンバー一覧","item":"{{ route('members.current') }}"}
        ]
    }
    </script>
...
@section('content')
    <nav class="text-sm text-gray-600 mt-2 px-4 max-w-6xl mx-auto" aria-label="パンくず">
        <ol class="flex flex-wrap items-center gap-2">
            ...
            <li>›</li>
            <li aria-current="page">現役メンバー一覧</li>
        </ol>
    </nav>

    <main class="container mx-auto mt-8 px-4 max-w-6xl">
        <header>
            <h1 class="text-2xl md:text-3xl font-semibold font-mont text-gray-900">日向坂46 現役メンバー一覧</h1>
            <p class="text-sm md:text-base text-gray-600 mt-2 leading-7">
                現在在籍している日向坂46メンバーを期別にまとめた一覧ページです。
        ...
        {{-- 一覧本体 --}}
        <section class="mt-10">
            <h2 class="text-xl font-semibold font-mont text-gray-800">現役メンバーを期別に掲載</h2>
            <p class="text-sm text-gray-600 mt-1">
                一期生から現在の各期まで、在籍メンバーを期別にまとめています。
            </p>
            ...
            @if (!$hasAnyMembers)
                <p class="mt-4 text-gray-700">現役メンバーはいません。</p>
            @else
                @foreach ($gradeOrder as $grade)
                    @php
                    ...
                    @if ($members->isNotEmpty())
                        <section class="mt-8" aria-labelledby="grade-{{ Str::slug($grade) }}">
                            <h3 id="grade-{{ Str::slug($grade) }}" class="text-lg font-semibold text-gray-900">
                                {{ $grade }}
                            </h3>
        ...
        <section class="mt-12 text-sm text-gray-700 leading-7 border-t pt-8">
            <h2 class="text-xl font-semibold text-gray-800 mb-3">日向坂46の現役メンバーを一覧で確認</h2>
            <p>
                このページでは、現在在籍している日向坂46メンバーを期別に掲載しています。
                メンバー全体を見たい方は
